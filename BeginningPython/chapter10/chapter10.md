...
函数random.randrange是生成随机整数的标准函数。为指定这个随机整数所在的范围，你可像调用range那样给这个函数提供参数。例如，要生成一个1~10(含)的随机整数，可 使用randrange(1, 11)或randrange(10) + 1。要生成一个小于20的随机正奇数，可使用randrange(1, 20, 2)。  
...
正则表达式是可匹配文本片段的模式。
+ 通配符
通配符包括`. `等。`.`匹配除换行符以外的所有字符。
  
+ 对特殊字符进行转义
对于一些在正则表达式中有特殊意义的字符，如果要将其作为普通字符，就需要进行转义比如要匹配`python.org`,就需要加`反斜线`对`.`进行转义。
...
+ 字符集
可以用方括号将一个子串括起来，创建一个所谓的字符集。这样的字符集与其包含的字符都匹配。**注意字符集只能匹配一个字符**
