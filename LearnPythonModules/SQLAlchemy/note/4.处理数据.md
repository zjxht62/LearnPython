...
## 使用SELECT语句
对于Core和ORM，select()函数生成一个Select结构，用于所有SELECT查询。传递给Core中的Connection.execute()和ORM中的Session.execute()等方法，
会在当前事务中发出SELECT语句，并通过返回的Result对象提供结果行。

### select() SQL表达式构造函数
select() 构造以与 insert() 相同的方式构建语句，使用生成方法，其中每种方法都会将更多状态添加到对象上。与其他 SQL 构造一样，它可以在原地转换为字符串：
```python
...
```
与所有其他语句级 SQL 构造一样，要实际运行该语句，我们将其传递给执行方法。由于 SELECT 语句返回行，我们始终可以迭代结果对象以获取 Row 个对象：
```python
with engine.connect() as conn:
...
ROLLBACK
```
在使用 ORM 时，特别是针对 ORM 实体组成的 select() 构造时，我们希望使用 Session 上的 Session.execute() 方法来执行它；通过这种方法，我们继续从结果中获得 Row 个对象，但是这些行现在可以包含完整的实体，比如 User 类的实例，作为每行中的单独元素：
```python
